...
void addFinalPath(RosMarker &line_list, Path<ConfigXY> &node);
...
int main(int argc, char **argv)
{
    ...
    RRT_Star<ConfigXY> rrt(kConfigSpace, kExtensionStepSize, kGoalSampleRate, kPathResolution, kNeighborhoodRadius);
    Path<ConfigXY> path = rrt.run(kStart, kGoal, 30);

    RosMarker start = createRosMarker(RosMarker::CUBE, ConfigXY(3.75, 5.52), {1, 0, 0});
    RosMarker goal = createRosMarker(RosMarker::CUBE, ConfigXY(4.29, 4.68), {0, 1, 0});

    RosMarker edges = createRosLineList({0, 0, 1});
    addTreeEdgesToMarker(edges, rrt.getStartNode());

    //RosMarker final_path = createRosLineStrip({0, 1, 0});
    //addFinalPath(final_path, path);
...
void addTreeEdgesToMarker(RosMarker &line_list, Node<ConfigXY> &node) {
    for (Node<ConfigXY> *child: node.getChildren()) {
        addLine(line_list, node.getConfig(), child->getConfig());
        addTreeEdgesToMarker(line_list, *child);
    }
}

void addFinalPath(RosMarker &line_strip, Path<ConfigXY> &path) {
    for (ConfigXY &config: path) {
        addPoint(line_strip, config);
    }
}